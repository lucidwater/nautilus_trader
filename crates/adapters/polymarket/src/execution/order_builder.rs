...
mod tests {
    ...
    fn test_build_market_buy_order_wire_shape() {
        // Locks the V2 market-BUY wire body: amount is quote (pUSD), so
        // maker_amount carries the quote spend and taker_amount carries the
        // computed share quantity. Expiration must always be "0" for market
        // orders, the builder code must be the empty bytes32, and a signature
        // must be produced. We don't pin the signature itself (no
        // py-clob-client-v2 vector available) but every structural invariant
        // a V2 endpoint checks before signature recovery is asserted here.
        let builder = make_test_builder();
        let order = builder
            .build_market_order(
                "71321045679252212594626385532706912750332728571942532289631379312455583992563",
    ...
    fn test_build_market_sell_order_wire_shape() {
        // SELL: amount is in shares, so maker_amount is shares and
        // taker_amount is the computed pUSD proceeds. Wire amounts are
        // 10^6-scaled mantissas, so 20 shares -> 20_000_000.
        let builder = make_test_builder();
        let order = builder
            .build_market_order(
                "71321045679252212594626385532706912750332728571942532289631379312455583992563",
